...
namespace CardFramework.Deck
{
    ...
    public class Set<T>
    {
        ...
        public Set(IRandomProvider random) : this(new List<T>(), random) {}
        ...
        public Set(IEnumerable<T> items, IRandomProvider randomProvider)
        {
            Items = new List<T>();
            AddToTop(items);
        ...
        public void Shuffle()
        {
            ...
            {
                int pos = _randomProvider.Range(i, Items.Count);
                T temp = Items[i];
                Items[i] = Items[pos];
                Items[pos] = temp;
            }
